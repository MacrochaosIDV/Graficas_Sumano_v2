...
void D3DXMesh::Create(std::string path_n_Name) {
	...
	//bdesc.ByteWidth = parser.v_Meshes[i].n_Vertexes * sizeof(CVertexMesh_p) * 4;
	...
	for (int i = 0; i < parser.n_meshes + 1; ++i) {
		...
		hr = D3D11Device->CreateBuffer(&bdesc, &subData, &parser.v_Meshes[i]._VB);
		if (hr != S_OK) {
		...
		for (int j = 0; j < parser.v_Meshes[i].n_mats; ++j) {
			...
			bdesc = { 0 };
			bdesc.ByteWidth = parser.v_Meshes[i].Materials[j].SubSet_IB.size() * sizeof(USHORT);
			bdesc.BindFlags = D3D11_BIND_INDEX_BUFFER;
			subData = { &parser.v_Meshes[i].Materials[j].SubSet_IB[0], 0, 0 };//Indexes
			hr = D3D11Device->CreateBuffer(&bdesc, &subData, &parser.v_Meshes[i].Materials[j]._IB);
			if (hr != S_OK) {
...
void D3DXMesh::Draw(float *t, float *vp) {
	...
	for (int i = 0; i < parser.n_meshes + 1; ++i) {
		...
		D3D11DeviceContext->IASetVertexBuffers(0, 1, parser.v_Meshes[i]._VB.GetAddressOf(), &stride, &offset);
		
		for (int j = 0; j < parser.v_Meshes[i].n_mats; ++j) {
			D3D11DeviceContext->IASetIndexBuffer(parser.v_Meshes[i].Materials[j]._IB.Get(), DXGI_FORMAT_R16_UINT, 0);
			D3D11DeviceContext->PSSetSamplers(0, 1, pSampler.GetAddressOf());
			D3D11DeviceContext->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
			D3D11DeviceContext->DrawIndexed(parser.v_Meshes[i].Materials[j].SubSet_IB.size(), 0, 0);
